...
In this paper, we describe the direct spectroscopic detection method in detail, and use it to detect the secondary star in 9 known binary systems. We describe the method in Section \ref{sec:method}. We describe the observations and data reduction in Section \ref{sec:obs}, then use the observations to estimate the accuracy with which we can measure the companion temperature in Section \ref{subsec:systematics} and the sensitivity of the method in Section \ref{subsec:sensitivity}. Finally, we use the direct spectroscopic method to search for known companions in Section \ref{sec:results}.
...
Here, we focus on detecting companions to rapidly rotating early-type stars, for which it is very difficult to detect the reflex motion of the primary star induced by a low-mass companion. We fit and remove the telluric absorption using the TelFit code \citep{Gullikson2014}, and estimate an empirical primary star spectrum with a Gaussian smoothing filter applied to the telluric-corrected data. We use a  window size ($w$) set by

\begin{equation}
w = \frac{v\sin{i} \cdot f}{c} \cdot \frac{\lambda_0}{\Delta \lambda}
\end{equation}
where $v\sin{i}$ is the literature rotational velocity of the star, $\lambda_0$ is the central wavelength of the echelle order, $\Delta \lambda$ is the wavelength spacing per pixel of the order, and $f = 0.25$ is an empirically determined parameter to give a visually adequate fit. Typical window sizes ranged from 50 - 100 pixels. 

We use the Phoenix library of model spectra prepared by \cite{Husser2013_b} throughout this work, and cross-correlate every echelle order that doesn't have strong telluric residuals against a grid of model spectra spanning the  parameters
...
\end{itemize}
Here, the $v\sin{i}$ is the rotational velocity of the secondary star. Estimating the primary star spectrum with a gaussian filter gives a much more accurate representation of the primary than a rotationally broadened model spectrum, but can distort the companion spectrum as well. We account for the small influence that the smoothing kernel has on the companion spectrum by convolving it with the same smoothing kernel used for the data, and subtracting the convolved model from the original. Treating the model spectrum in this way is more commonly known as unsharp masking. We rotationally broaden the model spectra and reduce the resolution and pixel sampling to that of the instrument \emph{before} the unsharp masking process. Finally, we combine the CCF s for all orders of a given observations using a simple average.
